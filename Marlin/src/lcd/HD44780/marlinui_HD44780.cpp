...
inline uint8_t draw_elapsed_or_remaining_time(uint8_t timepos, const bool blink) {
  ...
  if (!show_remain) {
    ...
    lcd_put_wchar(timepos, 2-1, LCD_STR_CLOCK[0]);
  }
...
//john!!!
    ...
    // ========== Line 3 ==========
      ...
      // lcd_put_wchar(0, 2-1, LCD_STR_FEEDRATE[0]);
      // lcd_put_u8str(i16tostr3rj(feedrate_percentage));
      // lcd_put_wchar('%');

      // const uint8_t timepos = 
      draw_elapsed_or_remaining_time(LCD_WIDTH - 8, blink);
